...
void NatHelper::onRequest(shared_ptr<RequestSipEvent>& ev) {
	...
	if (needToBeFixed(ev)) fixContactFromVia(ms->getHome(), sip, sip->sip_via);
	...
	if (rq->rq_method == sip_method_invite || rq->rq_method == sip_method_subscribe) {
		if (sip->sip_to->a_tag == NULL) {
			...
			if (mFixRecordRoutes) fixRecordRouteInRequest(ms);
...
bool NatHelper::needToBeFixed(const shared_ptr<SipEvent>& ev) {
...
bool NatHelper::empty(const char* value) {
...
void NatHelper::fixPath(shared_ptr<MsgSip>& ms) {
	sip_t* sip = ms->getSip();
...
void NatHelper::fixContactInResponse(su_home_t* home, msg_t* msg, sip_t* sip) {
...
void NatHelper::fixTransport(su_home_t* home, url_t* url, const char* transport) {
...
void NatHelper::fixContactFromVia(su_home_t* home, sip_t* msg, const sip_via_t* via) {
	...
	if (empty(received) && empty(rport)) return; /*nothing to do*/
	...
	for (; ctt != NULL; ctt = ctt->m_next) {
		if (ctt->m_url->url_host) {
			...
			url_param(ctt->m_url->url_params, "transport", ct_transport, sizeof(ct_transport) - 1);
			// If we have a single contact and we are the front-end proxy, or if we found a ip:port in a contact
			// that seems incorrect because the same appeared fixed in the via, then fix it.
			if ((is_frontend && single_contact) ||
			    (ModuleToolbox::urlHostMatch(host, via->v_host) && sipPortEquals(ctt->m_url->url_port, via->v_port) &&
...
void NatHelper::fixRecordRouteInRequest(shared_ptr<MsgSip>& ms) {
	sip_t* sip = ms->getSip();
