...
DataBaseEventLogWriter::DataBaseEventLogWriter(
	...
	try {
		...
		// Init tables.
		Backend backend = backendString == "mysql"
			? Backend::Mysql
			: (backendString == "sqlite3" ? Backend::Sqlite3 : Backend::Postgresql);
		{
...
void DataBaseEventLogWriter::writeRegistrationLog(soci::session *session, const std::shared_ptr<RegistrationLog> &evlog) {
	...
	*session << mInsertReq[SqlRegistrationEventLogId], soci::use(static_cast<int>(evlog->mType)), soci::use(contact);
}
...
void DataBaseEventLogWriter::writeMessageLog(soci::session *session, const std::shared_ptr<MessageLog> &evlog) {
	...
	*session << mInsertReq[SqlMessageEventLogId], soci::use(static_cast<int>(evlog->mReportType)), soci::use(uri);
}
