	...
	if (sip->sip_request->rq_url->url_params != NULL) {
		...
		if (url_has_param(url, "pn-provider")) {
			...
			try {
				...
				smatch base_match;
				if (regex_match(pn_provider, base_match, base_regex("apns(?:(.dev))*"))) {
			...
			try {
				...
				smatch base_match;
				if (regex_match(pn_param, base_match, base_regex("([A-Za-z0-9]+)\\.([a-z\\.]+)\\.([a-z&]+)"))) {
			...
			try {
				...
				for (const auto tokenAndService : tokenList) {
					if (tokenList.length() == 1) {
						if (regex_match(tokenAndService, base_match, base_regex("([a-z0-9]+)(?::([a-z]+))"))) {
							if (base_match.size() == 2) {
