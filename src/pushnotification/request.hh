...
enum class ApplePushType : std::uint8_t { Unknown, Pushkit, RemoteBasic, RemoteWithMutableContent, Background };
...
public:
	...
	std::string
	    mChatRoomAddr{}; // In case of a chat room invite, the sip addr of the chat room is needed. (ios specific).
	...
	bool needRinging() const noexcept {
		...
		return (mEvent == Event::Call || mEvent == Event::Refer) &&
		       (mApplePushType == ApplePushType::RemoteBasic ||
		        mApplePushType == ApplePushType::RemoteWithMutableContent);
	}
...
public:
	...
	const std::string& getAppIdentifier() const noexcept {
		return mAppId;
	}
