...
#include "router/routing-config.hh"
...
void ModuleRouter::onLoad(const GenericStruct* mc) {
	...
	routingConfig.mDomains = mReg->get<ConfigStringList>("reg-domains")->read();
	routingConfig.mAllowDomainRegistrations =
	    cr->get<GenericStruct>("inter-domain-connections")->get<ConfigBoolean>("accept-domain-registrations")->read();
	routingConfig.mUseGlobalDomain = mc->get<ConfigBoolean>("use-global-domain")->read();
	routingConfig.mAllowTargetFactorization = mc->get<ConfigBoolean>("allow-target-factorization")->read();

	const auto* fallbackRouteParam = mc->get<ConfigString>("fallback-route");
	routingConfig.mFallbackRoute = fallbackRouteParam->read();
	routingConfig.mFallbackRouteFilter = mc->get<ConfigBooleanExpression>("fallback-route-filter")->read();
	routingConfig.mFallbackParentDomain = mc->get<ConfigBoolean>("parent-domain-fallback")->read();

	if (!routingConfig.mFallbackRoute.empty()) {
		routingConfig.mFallbackRouteParsed =
		    ModuleToolbox::sipUrlMake(mHome.home(), routingConfig.mFallbackRoute.c_str());
		if (!routingConfig.mFallbackRouteParsed) throw BadConfigurationValue{fallbackRouteParam};
	}
	...
	routingConfig.mNoContactForAorReturnCode = noDevicesRegisteredReturnCodeParam->read();
	if (routingConfig.mNoContactForAorReturnCode < 400 || routingConfig.mNoContactForAorReturnCode > 599) {
		throw BadConfigurationValue{noDevicesRegisteredReturnCodeParam};
	}

	mResolveRoutes = mc->get<ConfigBoolean>("resolve-routes")->read();
	mDomains = routingConfig.mDomains;

	mForkManager = ForkManager::make(mAgent, this, mc, std::move(routingConfig));
}

void ModuleRouter::sendReply(
    Agent* agent, RequestSipEvent& ev, int code, const char* reason, int warn_code, const char* warning) {
	...
	if (warn_code != 0) {
		warn = sip_warning_format(ev.getHome(), "%i %s \"%s\"", warn_code, agent->getPublicIp().c_str(), warning);
	}
	if (warn) {
		ev.reply(code, reason, SIPTAG_SERVER_STR(agent->getServerString()), SIPTAG_WARNING(warn), TAG_END());
	} else {
		ev.reply(code, reason, SIPTAG_SERVER_STR(agent->getServerString()), TAG_END());
	}
}
		...
		while (iterator != NULL) {
			...
			if (getAgent()->isUs(route->r_url)) {
				...
				try {
					...
					mForkManager->fork(std::move(ev), requestUri);
					return {};
	...
	try {
		...
		if (sip->sip_request->rq_method == sip_method_cancel) {
			...
			if (transaction == nullptr) {
				sendReply(mAgent, *ev, SIP_481_NO_TRANSACTION);
				return std::move(ev);
			}
		...
		*/
		...
		sendReply(mAgent, *ev, SIP_100_TRYING);
