...
class Http2Client {
  public:
	...
	Http2Client(su_root_t& root, const std::string& host, const std::string& port, const SSL_METHOD* method);
	...
	bool isIdle() {
		return mActiveHttpContexts.empty() && mPendingRequests.empty();
	}
  ...
  private:
	...
	/* Private methods */
	ssize_t sendCb(nghttp2_session& session, const uint8_t* data, size_t length) noexcept;
	...
	void addTimeoutTimer(int32_t streamId);
	void onRequestTimeout(int32_t streamId);
	void resetTimeoutTimer(int32_t streamId);
	void removeTimeoutTimer(int32_t streamId);

	void connect();
	void disconnect();
	void processGoAwayOrErrors();
	...
	using HttpRequestList = std::list<std::shared_ptr<HttpMessageContext>>;
	HttpRequestList mPendingRequests;
	...
	// Delay (in second) for one request timeout
	static constexpr unsigned sRequestTimeout = 30;
};
