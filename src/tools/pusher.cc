...
struct PusherArgs {
	...
	void usage(const char *app) {
		cout << app << endl
			...
			<< "--pn-provider provider --pn-param params --pn-prid id" << endl
...
#define EQ1(i, name) (strcmp(name, argv[i]) == 0 && argc > i)
		...
		if (found_RFC_8599_Params) {
			smatch match;
			if (regex_match(standardPushParams.pnProvider, match, sApplePnProviderRegex)) {
				pntype = "apple";
			} else {
				cerr << "Error: standard push currently are only available for Apple, so pn-provider is expected to be 'apns' or 'apns.dev'. Found : " << standardPushParams.pnProvider << endl;
				exit(-1);
			}
		}
...
static vector<shared_ptr<Request>> createRequestFromArgs(const PusherArgs &args) {
	...
	if (args.legacyPush) { // Legacy push
		...
		PushInfo pinfo = createAndInitializePushInfo(args);
		// Only possible case for now is apple, else the parameter parsing would have exited
