...
int main(int argc, char *argv[]) {
	...
	*/
	...
	if (startPresence){
		...
		presenceServer = make_shared<flexisip::PresenceServer>(root->getCPtr());
	...
	if (startConference){
#ifdef ENABLE_CONFERENCE
		conferenceServer = make_shared<flexisip::ConferenceServer>(a->getPreferredRoute(), root->getCPtr());
		if (daemonMode) {
			notifyWatchDog();
		}
		try{
	...
	if (startRegEvent) {
#ifdef ENABLE_CONFERENCE
		regEventServer = make_shared<flexisip::RegistrationEvent::Server>(root->getCPtr());
		if (daemonMode) {
			notifyWatchDog();
		}
		try {
