...
FirebaseClient::FirebaseClient(su_root_t& root) {
	...
	mHttp2Client = make_unique<Http2Client>(root, FIREBASE_ADDRESS, FIREBASE_PORT, TLS_client_method());
}
...
void FirebaseClient::onError(const std::shared_ptr<HttpMessage>& request, int errorCode,
