...
void AccountPool::applyOperation(const CreateAccount& op) {
	...
	SLOGD << "AccountPool::CreateAccount - Creating account '" << accountDesc.uri << "' (alias='" << accountDesc.alias
	      << "')";
	...
	SLOGD << "AccountPool::CreateAccount - Account successfully created";
}
...
void AccountPool::applyOperation(const DeleteAccount& op) {
	...
	SLOGD << "AccountPool::DeleteAccount - Deleting account '"
	      << linphoneAccount->getParams()->getIdentityAddress()->asStringUriOnly() << "' (alias='"
	      << accountToDelete->getAlias().str() << "')";
...
void AccountPool::applyOperation(const UpdateAccount& op) {
	...
	SLOGD << "AccountPool::UpdateAccount - Updating account '" << oldIdentityAddress->asStringUriOnly() << "' (alias='"
...
void AccountPool::removeAccount(const std::shared_ptr<linphone::Account>& account) {
	account->addListener(make_shared<AuthInfoDeleter>(mCore));
...
void AccountPool::AuthInfoDeleter::onRegistrationStateChanged(const std::shared_ptr<linphone::Account>& account,
	...
	if (account == nullptr) {
		SLOGE << FUNC_LOG_PREFIX << "Received notification of registration state change (" << static_cast<int>(state)
		      << ") on empty account with message '" << message << "': abort";
		return;
	}
	const auto core = mCore.lock();
	if (core == nullptr) {
		SLOGD << FUNC_LOG_PREFIX
		      << "Cannot remove auth info, the Linphone::Core is not available anymore (std::weak_ptr is empty)";
		return;
	}
	...
	switch (state) {
		...
		case linphone::RegistrationState::Cleared: {
			SLOGD << FUNC_LOG_PREFIX << "Account '" << identity << "' unregistered with message '" << message
			      << "': removing auth info";
		} break;
		case linphone::RegistrationState::Failed: {
			SLOGD << FUNC_LOG_PREFIX << "Account '" << identity
			      << "' failed to update registration state with message '" << message << "': removing auth info";
		} break;
		default: {
			SLOGE << FUNC_LOG_PREFIX << "Unknown registration state '" << static_cast<int>(state) << "' for account '"
			      << identity << "' with message '" << message << "': removing auth info";
		}
	}
