...
static void complex_expressions(void) {
	...
	expr = SipBooleanExpressionBuilder::get().parse("((from.uri.user in 'jehan-kevin jehan-patrick') && user-agent "
	                                                "contains 'Linphone' ) || request.method == 'REGISTER'");
	BC_ASSERT_TRUE(expr != nullptr);
	BC_ASSERT_TRUE(expr->eval(getRequest()));
...
static void invalid_expressions(void) {
	...
	try {
		...
		expr = SipBooleanExpressionBuilder::get().parse("!(defined request.uri.user) && (from.uri.user in jehan-mac "
		                                                "jehan-michel) && user-agent contains 'Linphone'");
	} catch (...) {
	}
	BC_ASSERT_TRUE(expr == nullptr);
	...
	try {
		// mising parenthesis
		expr = SipBooleanExpressionBuilder::get().parse("!(defined request.uri.user) && (from.uri.user == 'jehan-mac'");
	} catch (...) {
	}
	BC_ASSERT_TRUE(expr == nullptr);
}
