...
class HttpMessageContext {
  public:
	...
	using OnResponseCb =
		std::function<void(const std::shared_ptr<HttpRequest> &, const std::shared_ptr<HttpResponse> &)>;
	using OnErrorCb =
		std::function<void(const std::shared_ptr<HttpRequest> &, const int error_code, const std::string &)>;
  ...
  private:
	...
	const OnResponseCb &mOnResponseCb;
	const OnErrorCb &mOnErrorCb;
};
