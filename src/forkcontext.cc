...
ForkContext::ForkContext(Agent *agent, const std::shared_ptr<RequestSipEvent> &event) :
		mAgent(agent), mEvent(make_shared<RequestSipEvent>(event)) {
...
bool ForkContext::onDestroy(const shared_ptr<IncomingTransaction> &transaction) {
	mIncoming.reset();
...
bool ForkContext::onDestroy(const shared_ptr<OutgoingTransaction> &transaction) {
