...
bool H264IFrameFilter::onOutgoingTransfer(uint8_t* data,
                                          size_t size,
	...
	if (isIFrame) {
		...
		if ((mIframeCount - 1) % mSkipCount != 0) ret = false;
	}

	return ret;
}

bool H264IFrameFilter::onIncomingTransfer([[maybe_unused]] uint8_t* data,
