...
void Http2Client::send(const shared_ptr<HttpRequest>& request, const OnResponseCb& onResponseCb,
	...
	auto context = make_shared<HttpMessageContext>(request, onResponseCb, onErrorCb, mRoot, requestTimeout);
	...
	context->getTimeoutTimer().set([this, streamId]() { this->onRequestTimeout(streamId); });
	resetIdleTimer();
}
...
void Http2Client::connect(shared_ptr<HttpMessageContext>& context) {
	...
	try {
		...
		if (!mConn->isConnected()) {
			string what{"TLS connection failed"};
			context->getOnErrorCb()(context->getRequest(), 503, what);
		...
		if ((status = nghttp2_submit_settings(session, NGHTTP2_FLAG_NONE, nullptr, 0)) != 0) {
			context->getOnErrorCb()(context->getRequest(), status, nghttp2_http2_strerror(status));
...
int Http2Client::onPollInCb(su_root_magic_t*, su_wait_t*, su_wakeup_arg_t* arg) noexcept {
	...
	if (thiz->mLastSID >= 0) {
		...
		for (auto it = thiz->mActiveHttpContexts.begin(); it != thiz->mActiveHttpContexts.end();
			 it = thiz->mActiveHttpContexts.erase(it)) {
			auto context = it->second;
			context->getOnErrorCb()(context->getRequest(), 503,
									"GOAWAY received, request may not be received by server.");
		}
		thiz->disconnect();
	}
	return 0;
