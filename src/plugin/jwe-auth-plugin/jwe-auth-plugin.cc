...
static vector<char> readFile(const string& path, bool* error = nullptr) {
	...
	} else SLOGW << "Unable to open: `" << path << "`.";
...
static list<string> listFiles(const string& path, const string& suffix) {
	...
	for (;;) {
		...
		if (!(dirent = readdir(dirp))) {
			if (errno) SLOGW << "Unable to read directory: `" << path << "` (" << strerror(errno) << ").";
			break;
		}

		string file(dirent->d_name);
		if (file != "." && file != ".." && endsWith(file, dotSuffix)) files.push_back(std::move(file));
	}
...
static bool isB64(const char* text, size_t len) {
	for (size_t i = 0; i < len; ++i)
		if (text[i] && !strchr(JOSE_B64_MAP, text[i])) return false;
	return true;
}
...
static json_t* parseJwe(const char* text) {
	...
	for (size_t i = 0, j = 0; j < nParts; ++j) {
		...
		size_t len;
		if (separator) len = separator - &text[i];
		else if (j + 1 == nParts) len = strlen(&text[i]);
		else goto error;

		if (!isB64(&text[i], len)) goto error;

		if (json_object_set_new(jwe, parts[j], json_stringn(&text[i], len)) < 0) goto error;
...
static const char* checkJwtTime(json_t* jwt, int* timeout) {
	...
	*timeout = numeric_limits<int>::max();
	...
	if ((attrValue = json_object_get(jwt, "exp"))) {
		...
		if (json_typeof(attrValue) != JSON_INTEGER) return "Invalid exp attr, must be an integer";

		if (json_unpack(attrValue, "I", &expValue) < 0) return "Unable to extract exp attr";

		if (time_t(expValue) < currentTime) return "exp has expired";
	...
	if ((attrValue = json_object_get(jwt, "exp_in"))) {
		...
		if (json_typeof(attrValue) != JSON_INTEGER) return "Invalid exp_in attr, must be an integer";

		if (json_unpack(attrValue, "I", &expInValue) < 0) return "Unable to extract exp_in attr";

		json_int_t iatValue;
		if (json_unpack(jwt, "{s:I}", "iat", &iatValue) < 0)
			return "Unable to extract iat attr, must be an integer and exists";

		if (time_t(iatValue) + expInValue < currentTime) return "exp_in has expired";

		attrChecked = true;
		*timeout = min(*timeout, int(time_t(iatValue) + expInValue - currentTime));
	}

	if (!attrChecked) return "exp and/or exp_in attr must exists";
...
static bool checkJwtAttrFromSipHeader(json_t* jwt, const sip_t* sip, const char* attrName, const char* sipHeaderName) {
	...
	if (json_unpack(jwt, "{s:s}", attrName, &value) == 0) {
		...
		if (header && header->un_value) soFarSoGood = !strcmp(value, header->un_value);
	}
	return soFarSoGood;
}
...
void JweAuth::onRequest(shared_ptr<RequestSipEvent>& ev) {
	...
	if (!(header = ModuleToolbox::getCustomHeaderByName(sip, mOidCustomHeader.c_str())) || !header->un_value)
	...
	else if (!ev->findIncomingSubject(header->un_value)) error = "Unable to match oid";
		...
		if (it == mJweContexts.end()) {
			...
			if (!jwt) error = "Unable to decrypt JWE";
			...
			if (jweContext->consumed) error = "JWE already consumed";
		}
	}
...
void JweAuth::insertJweContext(string&& jweKey, const shared_ptr<JweContext>& jweContext, int timeout) {
	...
	if (timeout > 0) {
		timeout *= 1000;
		if (timeout < 0) timeout = numeric_limits<int>::max();
	}
