...
public:
	ForkMessageContextDbProxy(Agent* agent, const std::shared_ptr<RequestSipEvent>& event,
	                          std::shared_ptr<ForkContextConfig> cfg, std::weak_ptr<ForkContextListener> listener,
	                          std::weak_ptr<StatPair> counter)
	    : mOriginListener{listener} {
		mForkMessage = std::make_unique<ForkMessageContext>(agent, event, cfg, shared_from_this(), counter);
	}
...
private:
	...
	mutable std::unique_ptr<ForkMessageContext> mForkMessage;
