...
if sys.version_info.major < 3:
    print(
        "ERROR: current Python version is {0}.{1}.{2} whereas Python 3 is required.".format(
...
class Version:
    def __init__(
        ...
        if fromStr is not None:
            m = re.match(
                "^([0-9]+)\\.([0-9]+)\\.([0-9]+)(-alpha|-beta|-pre)?((-[0-9]+)(-g[0-9a-f]+))?$",
                fromStr,
            )
            if m is None:
                raise ValueError(
                    "'{0}' isn't a valid git describe string".format(fromStr)
                )
            major = m.group(1)
            minor = m.group(2)
            patch = m.group(3)
...
class FlexisipServer:
    ...
    def _get_version(self):
        p = subprocess.Popen(
            [self.path, "-v"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        out, err = p.communicate()
        out = str(out, encoding="utf-8")
        m = re.search("version: ([.a-z0-9-]+)", out)
        if m is None:
            raise RuntimeError(f"unexpected output of 'flexisip -v': [{out}]")
        version = m.group(1)
        try:
            return Version(fromStr=version)
        except ValueError:
            raise RuntimeError(
                f"invalid version string in output of 'flexisip -v' [{version}]"
            )
...
class XWiki:
    class Credential:
        ...
        def to_base64(self):
            return base64.b64encode(
                bytes("{0}:{1}".format(self.user, self.password), encoding="utf-8")
            )
    ...
    def update_page(self, path, content):
        ...
        if response.status not in (201, 202):
            raise RuntimeError(
                "page creation or modification has failed"
    ...
    def _forge_root_uri(self):
        return (
            self.scheme + "://" + self.host + XWiki._apipath + "/wikis/" + self.wikiname
        )
    ...
    def _forge_http_request(self, uri, method, body):
        ...
        if self.credentials is not None:
            headers["Authorization"] = "Basic " + str(
                self.credentials.to_base64(), encoding="ascii"
            )
        return urllib.request.Request(
            uri, data=bytes(body, encoding="utf-8"), headers=headers, method=method
        )


class DocWriter:
    def __init__(self, wiki, server, wiki_directory):
        self.wiki = wiki
        self.server = server
        self.document_root = wiki_directory

    def write_and_push(self):
        childrenMacro = "{{children/}}"
        self.wiki.update_page(
            os.path.join(self.document_root, "WebHome"),
            '{{children sort=":desc"/}}',
        )
        self.wiki.update_page(
            os.path.join(self._get_version_page_path(), "WebHome"), childrenMacro
        )
        self.wiki.update_page(
            os.path.join(self._get_version_page_path(), "module/WebHome"), childrenMacro
        )

        for section in self.server.section_list:
            ...
            print(f"Updating page '{path}'")
            self.wiki.update_page(path, out)

    def _section_name_to_page_path(self, module_name):
        return os.path.join(
            self._get_version_page_path(), *tuple(module_name.split("::"))
        )

    def _get_version_page_path(self):
        if self.server.version.branch == "alpha":
            version = "master"
        elif self.server.version.branch is None or self.server.version.branch == "beta":
            version = self.server.version.short_version
        else:
            raise RuntimeError(
                f"Reference documentation isn't authorized to be pushed for this version of Flexisip [{self.server.version.git_version}]"
            )
        return os.path.join(self.document_root, version)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Send the Flexisip documentation to the Wiki. All options passed override the config file."
    )
    parser.add_argument(
        "-H",
        "--host",
        help="the host to which we should send the documentation",
        default="wiki.linphone.org:8080",
    )
    parser.add_argument(
        "-w",
        "--wiki",
    ...
    parser.add_argument(
        "-u",
        "--user",
        help="the user to authenticate to the server",
        dest="config_user",
    )
    parser.add_argument(
        "-p",
        "--password",
        help="the password to authenticate to the server",
        dest="config_password",
    )
    parser.add_argument(
        "--cafile",
        help="file containing a set of trusted certificates",
        default=None,
    )
    parser.add_argument(
        "--flexisip-binary",
        help="location of the Flexisip executable to run",
        default="../OUTPUT/bin/flexisip",
    )
    parser.add_argument(
        "-d",
        "--wiki-directory",
    ...
    if settings.password == "":
        print(
            "Please define a password using "
            + config_file
    ...
    credentials = XWiki.Credential(settings.user, settings.password)
    wiki = XWiki(
        settings.host,
        settings.wikiname,
    ...
    server = FlexisipServer(args.flexisip_binary)
    docWriter = DocWriter(wiki, server, args.wiki_directory)
