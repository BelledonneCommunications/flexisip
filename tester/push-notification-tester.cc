...
static void startFirebasePushTest(const PushInfo& pushInfo, const string& reqBodyPattern, int responseCode,
	...
	ApnsMock apnsMock;
	auto	 serverReturn =
		std::async(std::launch::async, [&apnsMock, responseCode, &responseBody, &reqBodyPattern, timeout]() {
			return apnsMock.exposeMock(responseCode, responseBody, reqBodyPattern, timeout);
		});

	// Give time to the mock server to run before sending the request
	...
	// FirebaseClient::onResponse/onError is called and response status is well managed
	...
	if (!timeout) {
		BC_ASSERT_TRUE(serverReturn.get() == true);
	}
}
...
static void startApplePushTest(const PushInfo& pushInfo, const string& reqBodyPattern, int responseCode,
	...
	AppleClient appleClient{*root, "", "/home/anthony/source/flexisip/tester/cert/apple.test.dev.pem",
							"apple.test.dev.pem"};
	...
	// AppleClient::onResponse/onError is called and response status is well managed
	BC_ASSERT_TRUE(request->getState() == expectedFinalState);
