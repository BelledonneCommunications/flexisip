...
shared_ptr<B2buaCore> B2buaCore::create(linphone::Factory& factory, const GenericStruct& config) {
	...
	if (string cVideoCodec = config.get<ConfigString>("video-codec")->read(); !cVideoCodec.empty()) {
		// disable all video codecs
		for (const auto& pt : core->getVideoPayloadTypes()) {
			BCTBX_SLOGI << "Disable " << pt->getMimeType() << " codec as only " << cVideoCodec << " should be used";
			pt->enable(false);
		}
		// enable the given one
		auto enabledCodec = core->getPayloadType(cVideoCodec, -1, -1);
		if (enabledCodec) {
