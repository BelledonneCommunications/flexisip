...
FirebaseRequest::FirebaseRequest(const PushInfo& pinfo) : Request(pinfo.mAppId, "firebase") {
	...
	copy(formatedBody.begin(), formatedBody.end(), back_inserter(mBody));
	...
	hStore.add(":path", "/fcm/send");
	hStore.add(":authority", string(FirebaseClient::FIREBASE_ADDRESS) + ":" + string(FirebaseClient::FIREBASE_PORT));
	hStore.add("content-type", "application/json");
	hStore.add("authorization", "key=" + apiKey);
	this->setHeaderStore(hStore);
