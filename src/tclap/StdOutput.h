								...
								ArgException& e )
{
	...
	std::cerr << "PARSE ERROR: " << e.argId() << ": " << e.error() << std::endl;
						...
						std::ostream& os ) const
{
	...
	if ( secondLineOffset > 80/2 )
		secondLineOffset = static_cast<int>(80/2);

	spacePrint( os, s, 80, 3, secondLineOffset );
}
	...
	std::ostream& os ) const
{
	...
	for ( int i = 0; static_cast<unsigned int>(i) < xorList.size(); i++ )
	{
		for ( ArgVectorIterator it = xorList[i].begin();
			...
			it++ )
		{
			// os << "  " << (*it)->longID() << ":" << (*it)->getDescription() << std::endl;
			spacePrint( os, (*it)->longID(), 80, 3, 3 );
						           ...
						           int firstLineOffset ) const
{
	...
	if ( (len + indentSpaces > maxWidth - firstLineOffset) && maxWidth > 0 )
		{
			int allowedLen = maxWidth - indentSpaces - firstLineOffset;
			int start = 0;
			while ( start < len )
				{
					...
					if ( start == 0 )
						{
							...
							// adjust allowed len
							allowedLen -= secondLineOffset - firstLineOffset;
						}
