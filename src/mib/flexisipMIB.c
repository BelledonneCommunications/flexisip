...
void init_flexisipMIB(Agent& agent, ConfigManager& aCm) {
	...
	netsnmp_register_scalar(netsnmp_create_handler_registration("ipAddress", handle_ipAddress, ipAddress_oid,
	                                                            OID_LENGTH(ipAddress_oid), HANDLER_CAN_RONLY));
	netsnmp_register_scalar(netsnmp_create_handler_registration("bindAddress", handle_bindAddress, bindAddress_oid,
	                                                            OID_LENGTH(bindAddress_oid), HANDLER_CAN_RONLY));
	netsnmp_register_scalar(
	    netsnmp_create_handler_registration("port", handle_port, port_oid, OID_LENGTH(port_oid), HANDLER_CAN_RONLY));
	netsnmp_register_scalar(netsnmp_create_handler_registration(
	    "transportUris", handle_transportUris, transportUris_oid, OID_LENGTH(transportUris_oid), HANDLER_CAN_RONLY));
	netsnmp_register_scalar(netsnmp_create_handler_registration(
	    "totalNumberOfAddedRecords", handle_totalNumberOfAddedRecords, totalNumberOfAddedRecords_oid,
	    OID_LENGTH(totalNumberOfAddedRecords_oid), HANDLER_CAN_RONLY));
	netsnmp_register_scalar(netsnmp_create_handler_registration(
	    "totalNumberOfExpiredRecords", handle_totalNumberOfExpiredRecords, totalNumberOfExpiredRecords_oid,
...
int handle_ipAddress(netsnmp_mib_handler* handler,
                     netsnmp_handler_registration* reginfo,
                     netsnmp_agent_request_info* reqinfo,
                     netsnmp_request_info* requests) {
	/* We are never called for a GETNEXT if it's registered as a
...
int handle_bindAddress(netsnmp_mib_handler* handler,
                       netsnmp_handler_registration* reginfo,
                       netsnmp_agent_request_info* reqinfo,
                       netsnmp_request_info* requests) {
	/* We are never called for a GETNEXT if it's registered as a
...
int handle_transportUris(netsnmp_mib_handler* handler,
                         netsnmp_handler_registration* reginfo,
                         netsnmp_agent_request_info* reqinfo,
                         netsnmp_request_info* requests) {
	/* We are never called for a GETNEXT if it's registered as a
	...
	switch (reqinfo->mode) {
		...
		case MODE_GET:
			snmp_set_var_typed_value(requests->requestvb, ASN_OCTET_STR, ag->getTransportUri().c_str(),
...
int handle_totalNumberOfAddedRecords(netsnmp_mib_handler* handler,
                                     netsnmp_handler_registration* reginfo,
                                     netsnmp_agent_request_info* reqinfo,
                                     netsnmp_request_info* requests) {
	/* We are never called for a GETNEXT if it's registered as a
	...
	switch (reqinfo->mode) {
		case MODE_GET: {
			unsigned long value =
			    dynamic_cast<RegistrarMgt&>(ag->getModuleByName("Registrar")).getTotalNumberOfAddedRecords();
...
int handle_totalNumberOfExpiredRecords(netsnmp_mib_handler* handler,
                                       netsnmp_handler_registration* reginfo,
                                       netsnmp_agent_request_info* reqinfo,
                                       netsnmp_request_info* requests) {
	/* We are never called for a GETNEXT if it's registered as a
	...
	switch (reqinfo->mode) {
		...
		case MODE_GET: {
			unsigned long value =
			    dynamic_cast<RegistrarMgt&>(ag->getModuleByName("Registrar")).getTotalNumberOfExpiredRecords();
			struct counter64 conter;
			conter.high = value >> 32;
