...
PushParamList::PushParamList(const string& provider, const string& customPrId, const string& customParam)
	...
	vector<string> splitedPrId = StringUtils::split(customPrId, "&");
...
ostream& operator<<(ostream& os, const PushParam& pushParam) const noexcept {
	os << "PushParam[" << pushParam.getPrId() << ", " << pushParam.getParam() << ']' << endl;
	return os;
}

ostream& operator<<(ostream& os, const PushParamList& pushParamList) const noexcept {
