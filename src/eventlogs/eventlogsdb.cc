...
CallQualityStatisticsLogDb::CallQualityStatisticsLogDb(const std::shared_ptr<CallQualityStatisticsLog> &csLog)
	...
	while (iss >> token) {
		...
		if (token == "CallID:") {
			...
			if (key == "IP")
				current_addr->ip = value;
			else if (key == "PORT")
				current_addr->port = atoi(value.c_str());
			else if (key == "SSRC")
				current_addr->ssrc = (unsigned int)atoi(value.c_str());
			else if (section == "Timestamps") {
				time_t *ts = NULL;
				if (key == "START")
					ts = &current_metrics->ts_start;
				else if (key == "STOP")
					ts = &current_metrics->ts_stop;
				...
				if (ts) {
					...
					*ts = mktime(&tm);
					...
					SLOGE << "CallQualityStatisticsLogDb: Unhandled key=" << key << " value=" << value
						  << " in section=" << section;
				}
			} else if (section == "SessionDesc" && key == "PT")
				current_metrics->sd_payload_type = atoi(value.c_str());
			else if (section == "SessionDesc" && key == "PD")
				current_metrics->sd_payload_desc = value;
			else if (section == "SessionDesc" && key == "SR")
				current_metrics->sd_sample_rate = atoi(value.c_str());
			else if (section == "SessionDesc" && key == "FD")
				current_metrics->sd_frame_duration = atoi(value.c_str());
			else if (section == "SessionDesc" && key == "FMTP")
				current_metrics->sd_fmtp = value;
			else if (section == "SessionDesc" && key == "PLC")
				current_metrics->sd_packet_loss_concealment = atoi(value.c_str());
			else if (section == "JitterBuffer" && key == "JBA")
				current_metrics->jb_adaptive = atoi(value.c_str());
			else if (section == "JitterBuffer" && key == "JBN")
				current_metrics->jb_nominal = atoi(value.c_str());
			else if (section == "JitterBuffer" && key == "JBM")
				current_metrics->jb_max = atoi(value.c_str());
			else if (section == "JitterBuffer" && key == "JBX")
				current_metrics->jb_abs_max = atoi(value.c_str());
			else if (section == "PacketLoss" && key == "NLR")
				current_metrics->pl_network_packet_loss_rate = atoi(value.c_str());
			else if (section == "PacketLoss" && key == "JDR")
				current_metrics->pl_jitter_buffer_discard_rate = atoi(value.c_str());
			else if (section == "Delay" && key == "RTD")
				current_metrics->d_round_trip_delay = atoi(value.c_str());
			else if (section == "Delay" && key == "ESD")
				current_metrics->d_end_system_delay = atoi(value.c_str());
			else if (section == "Delay" && key == "IAJ")
				current_metrics->d_interarrival_jitter = atoi(value.c_str());
			else if (section == "Delay" && key == "MAJ")
				current_metrics->d_mean_abs_jitter = atoi(value.c_str());
			else if (section == "Signal" && key == "SL")
				current_metrics->s_level = atoi(value.c_str());
			else if (section == "Signal" && key == "NL")
				current_metrics->s_noise_level = atoi(value.c_str());
			else if (section == "QualityEst" && key == "MOSLQ")
				current_metrics->qe_moslq = atof(value.c_str());
			else if (section == "QualityEst" && key == "MOSCQ")
				current_metrics->qe_moscq = atof(value.c_str());
			else if (section == "LinphoneExt" && key == "UA")
				current_metrics->user_agent = value;
			else if (section == "AdaptiveAlg" && key == "NAME")
				qos_name = value;
			else if (section == "AdaptiveAlg" && key == "TS")
				qos_timestamp = value;
			else if (section == "AdaptiveAlg" && key == "IN_LEG")
				qos_input_leg = value;
			else if (section == "AdaptiveAlg" && key == "IN")
				qos_input = value;
			else if (section == "AdaptiveAlg" && key == "OUT_LEG")
				qos_output_leg = value;
			else if (section == "AdaptiveAlg" && key == "OUT")
				qos_output = value;
			else
				SLOGE << "CallQualityStatisticsLogDb: Unhandled key=" << key << " value=" << value
					  << " in section=" << section;
