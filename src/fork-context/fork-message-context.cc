...
void ForkMessageContext::assertEqual(const shared_ptr<ForkMessageContext>& expected) {
	...
	auto isSameSize = equal(mWaitingBranches.begin(), mWaitingBranches.end(), expected->mWaitingBranches.begin(),
	                        [](const auto& a, const auto& b) {
		                        a->assertEqual(b);
		                        // Always return true because assertEqual already check for element equality, equal(...)
		                        // only check size
		                        return true;
	                        });
	BC_ASSERT_TRUE(isSameSize);
}
#endif