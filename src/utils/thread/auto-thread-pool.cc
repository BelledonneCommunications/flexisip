...
AutoThreadPool::AutoThreadPool(unsigned int maxThreadNumber, unsigned int maxQueueSize)
	...
	SLOGD << "AutoThreadPool[" << this << "]: Created with " << maxThreadNumber << " threads and queue size "
	      << maxQueueSize;
		...
		{ // Scope based locking.
			...
			mCondition.wait(lock, [this]() {
				if (mCurrentThreadNumber == mMaxThreadNumber) {
					SLOGD << "AutoThreadPool[" << this << "]: Notified but the maximum number (" << mMaxThreadNumber
			...
			if (mTasks.empty() && mState == Shutdown) {
				SLOGD << "AutoThreadPool[" << this << "]: Terminating " << mCurrentThreadNumber << " threads";
