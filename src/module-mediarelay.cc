...
void MediaRelay::onDeclare(GenericStruct *mc) {
	ConfigItemDescriptor items[] = {
		{String, "nortpproxy", "SDP attribute set by the first proxy to forbid subsequent proxies to provide relay. "
		...
		{Boolean, "bye-orphan-dialogs",
		 "Sends a ACK and BYE to 200Ok for INVITEs not belonging to any established call.", "false"},
		{Integer, "max-calls", "Maximum concurrent calls processed by the media-relay. Calls arriving when the limit "
							   "is exceed will be rejected. "
		...
		{Boolean, "early-media-relay-single",
		 ...
		 "streams forwarded back to caller. This feature prevents the caller to receive 'mixed' streams, but it breaks "
		 "scenarios where multiple servers play early media announcement in sequence.",
		 "true"},
		{Integer, "max-early-media-per-call", "Maximum number of relayed early media streams per call. This is useful "
											  "to limit the cpu usage due to early media relaying on"
											  " embedded systems. A value of 0 stands for unlimited.",
		 "0"},
#ifdef MEDIARELAY_SPECIFIC_FEATURES_ENABLED
		/*very specific features, useless for most people*/
		{Integer, "h264-filtering-bandwidth", "Enable I-frame only filtering for video H264 for clients annoucing a "
											  "total bandwith below this value expressed in kbit/s. Use 0 to disable "
											  "the feature",
		 "0"},
...
void MediaRelay::onLoad(const GenericStruct *modconf) {
	...
	if (mSdpMangledParam == "disable")
		mSdpMangledParam.clear();
...
bool MediaRelay::processNewInvite(const shared_ptr<RelayedCall> &c, const shared_ptr<OutgoingTransaction> &transaction,
	...
	c->initChannels(m, from_tag, transaction->getBranchId(), mAgent->getPreferredIp(from_host),
					mAgent->getPreferredIp(dest_host));
	...
	m->iterate(
		bind(&RelayedCall::setChannelDestinations, c, m, _1, _2, _3, from_tag, transaction->getBranchId(), false));
...
void MediaRelay::onRequest(shared_ptr<RequestSipEvent> &ev) {
	...
	if (sip->sip_request->rq_method == sip_method_invite) {
		...
		if (c == NULL)
			c = dynamic_pointer_cast<RelayedCall>(mCalls->find(getAgent(), sip, false));
		if (c == NULL) {
...
void MediaRelay::processResponseWithSDP(const shared_ptr<RelayedCall> &c,
	...
	m->iterate(
		bind(&RelayedCall::setChannelDestinations, c, m, _1, _2, _3, to_tag, transaction->getBranchId(), isEarlyMedia));
	// modify sdp
...
void MediaRelay::onResponse(shared_ptr<ResponseSipEvent> &ev) {
	...
	if (ot == NULL && it == NULL && sip->sip_cseq && sip->sip_cseq->cs_method == sip_method_invite &&
