...
typedef logging::aux::light_function<void(logging::record_view const &, logging::basic_formatting_ostream<char> &)>
	formatter_functor;
...
typedef boost::function2<void, std::basic_ostream<char> &, boost::log::basic_record<char> const &> formatter_functor;
...
#define addIfString(name, before, after)                                                                               \
	fmt::if_(flt::has_attr(name))[fmt::stream << before << fmt::attr<std::string>(name) << after]

//! Formatter functor
formatter_functor createFormatter(bool timestamp = true) {
	return fmt::stream
		   << fmt::if_(flt::has_attr(
				  "TimeStamp"))[fmt::stream << "[" << FMTDATETIME("TimeStamp", "%d.%m.%Y %H:%M:%S.%f") << "] "]
		   << fmt::if_(
				  flt::has_attr("Severity"))[fmt::stream << "[" << fmt::attr<flexisip::log::level>("Severity") << "]"]
#if (BOOST_VERSION >= 105400)
		   << " [" << fmt::attr<logging::attributes::current_thread_id::value_type>("ThreadID") << "]"
#endif
		   << addIfString("method_or_status", " [", "]") << addIfString("Module", " [", "]")
		   << addIfString("callid", " [", "]") << " : " << EXPR_MESSAGE;
}
...
static void init_log_to_syslog() {
	...
	boost::shared_ptr<back_type> sink(new back_type(keywords::use_impl = sinks::syslog::native));
