...
namespace flexisip {
	...
	ListSubscription::ListSubscription(unsigned int expires, belle_sip_server_transaction_t* ist,belle_sip_provider_t* aProv) throw (FlexisipException):
	...
	,mTimer(NULL) {
		...
		for (::resource_lists::List::ListConstIterator listIt =resource_list_body->getList().begin();listIt !=resource_list_body->getList().end();listIt++) {
			for (::resource_lists::List::EntryConstIterator entryIt =listIt->getEntry().begin();entryIt !=listIt->getEntry().end();entryIt++) {
	...
	list<shared_ptr<PresentityPresenceInformationListener>>& ListSubscription::getListeners() {
		shared_ptr<PresentityPresenceInformationListener> toto = make_shared<PresentityResourceListener>(*this,belle_sip_uri_new());
		return mListeners;
	}
	...
	void ListSubscription::addInstanceToResource(rlmi::Resource& resource, list <belle_sip_body_handler_t*>& multipartList, PresentityPresenceInformation& presentityInformation) {
		...
		belle_sip_memory_body_handler_t *bodyPart = belle_sip_memory_body_handler_new_copy_from_buffer((void *)pidf.c_str()
																								  ,pidf.length()
																								  , NULL
																								  , NULL);
		belle_sip_body_handler_add_header(BELLE_SIP_BODY_HANDLER(bodyPart), belle_sip_header_create("Content-Transfer-Encoding", "binary"));
		ostringstream content_id;
		content_id <<"<"<<cid.str()<<">";
		belle_sip_body_handler_add_header(BELLE_SIP_BODY_HANDLER(bodyPart), belle_sip_header_create("Content-Id", cid.str().c_str()));
		belle_sip_body_handler_add_header(BELLE_SIP_BODY_HANDLER(bodyPart), belle_sip_header_create("Content-Type", "application/pidf+xml;charset=\"UTF-8\""));
	...
	void ListSubscription::notify(bool_t isFullState) throw(FlexisipException) {
		...
		try {
			...
			belle_sip_memory_body_handler_t *fistBodyPart = belle_sip_memory_body_handler_new_copy_from_buffer((void *)out.str().c_str()
																										  ,out.str().length()
																										  , NULL
																										  , NULL);
			belle_sip_body_handler_add_header(BELLE_SIP_BODY_HANDLER(fistBodyPart), belle_sip_header_create("Content-Transfer-Encoding", "binary"));
			ostringstream content_id;
			content_id <<"<"<<cid.str()<<">";
		...
		if (getState() == active) {
			...
			if (isTimeToNotify ()) {
				...
				if (mVersion >0 /*special case for first notify */ && mTimer == NULL) {
					...
					// create timer
					chrono::milliseconds timeout(chrono::duration_cast<chrono::milliseconds>(mMinNotifyIntervale - (chrono::system_clock::now()-mLastNotify)));
					
					mTimer = belle_sip_main_loop_create_timeout(belle_sip_stack_get_main_loop(belle_sip_provider_get_sip_stack(mProv))
				...
				if (mVersion >0) {
					SLOGI <<"Defering presence information notify for entity ["<<presenceInformation.getEntity()<<"] to [" << (belle_sip_source_get_timeout(mTimer))<< " ms]";
				} else {
