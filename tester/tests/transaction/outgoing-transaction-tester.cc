...
void iterateServerAndClient(Server& server, BellesipUtils& client, const chrono::duration<double> duration) {
	const auto& root = server.getRoot();
...
void resilienceToNetworkError() {
	...
	const auto& records = dynamic_cast<const RegistrarDbInternal*>(regDb)->getAllRecords();
...
Content-Length: 0)sip");
	...
	iterateServerAndClient(proxy, unstableClient, 2s);
	BC_ASSERT_CPP_NOT_EQUAL(ackCounter, 0);
	BC_ASSERT_CPP_NOT_EQUAL(outgoingTransaction->getResponseCode(), 0xaaaa);
}
