		...
		if (cseq && (cseq->cs_method == sip_method_invite || cseq->cs_method == sip_method_subscribe)) {
			if (st->st_status >= 200 && st->st_status <= 299) {
				...
				if (ct) {
					...
					bool isLastHop = ms->getSip()->sip_via && ms->getSip()->sip_via->v_next &&
					                 !ms->getSip()->sip_via->v_next->v_next;
					if (isLastHop) {
						if (isVerified) {
							// Via contains client and first proxy
							LOGD("Removing verified param from response contact");
							ct->m_url->url_params = url_strip_param_string(
							    su_strdup(ms->getHome(), ct->m_url->url_params), mContactVerifiedParam.c_str());
						}
					} else {
						if (needToBeFixed(ev)) {
