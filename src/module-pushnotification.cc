	...
	if (pinfo.mType == "apple") {
		if (mAppleForcePnType == pushnotification::ApplePushType::Unknown) {
			...
			if (pinfo.mApplePushType == pushnotification::ApplePushType::Pushkit && !endsWithVoip) {
				LOGD("requiring PushKit notification with invalid app-id[%s]. Inserting 'voip' part", appId.c_str());
				...
				LOGD("requiring Remote push notification with invalid app-id[%s]. Removing 'voip' part", appId.c_str());
