...
template <typename _valuesT>
std::shared_ptr<ExpressionElement> BooleanExpressionBuilder<_valuesT>::buildElement(const std::string& expr,
                                                                                    size_t* newpos) {
...
std::shared_ptr<BooleanExpression<_valuesT>> BooleanExpressionBuilder<_valuesT>::parse(const std::string& expression) {
	if (expression.empty())
		return make_shared<ConstantBooleanExpression<_valuesT>>(
		    true); /* By arbitrary decision, we evaluate void to true.*/
	size_t pos = 0;
...
template <typename _valuesT>
shared_ptr<BooleanExpression<_valuesT>>
BooleanExpressionBuilder<_valuesT>::parseExpression(const string& expr, size_t* newpos, bool immediateNeighbour) {
	size_t i;
