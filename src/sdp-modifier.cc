...
std::list<PayloadType *> SdpModifier::findCommon(const std::list<PayloadType *> &offer,
												 const std::list<PayloadType *> &answer, bool use_offer_numbering) {
...
short SdpModifier::getAudioIpVersion() {
	...
	if (c && c->c_addrtype == sdp_addr_ip6)
		return 6;
	return 4;
}
...
void SdpModifier::addIceCandidate(std::function<std::pair<std::string, int>(int)> getRelayAddrFcn,
	...
	for (i = 0; mline != NULL; mline = mline->m_next, ++i) {
		if (hasMediaAttribute(mline, "candidate") && !hasMediaAttribute(mline, "remote-candidates") &&
			!hasMediaAttribute(mline, mNortproxy.c_str())) {
			uint32_t priority;
			...
			for (uint16_t componentID = 1; componentID <= 2; componentID++) {
				...
				if (!hasIceCandidate(mline, relayAddr.first.c_str(), relayAddr.second + componentID - 1)) {
					...
					candidate_line << foundation << ' ' << componentID << " UDP " << priority << ' '
								   << relayAddr.first.c_str() << ' ' << relayAddr.second + componentID - 1
								   << " typ relay raddr " << destAddr.first << " rport "
								   << destAddr.second + componentID - 1;
					addMediaAttribute(mline, "candidate", candidate_line.str().c_str());
				}
			}
			if (!mNortproxy.empty())
				addMediaAttribute(mline, mNortproxy.c_str(), "yes");
		}
	}
}
...
void SdpModifier::iterate(function<void(int, const string &, int)> fct) {
	...
	for (i = 0; mline != NULL; mline = mline->m_next, ++i) {
		...
		if (hasMediaAttribute(mline, mNortproxy.c_str()))
			continue;
...
bool SdpModifier::hasIceCandidate(sdp_media_t *mline, const string &addr, int port) {
	...
	while ((candidate = sdp_attribute_find(candidate, "candidate")) != NULL) {
		...
		stream >> type;
		if ((candidate_port == port) && (addr.compare(candidate_addr) == 0))
			return true;
