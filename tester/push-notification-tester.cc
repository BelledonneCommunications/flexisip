...
static void startFirebasePushTest(const PushInfo& pushInfo, const string& reqBodyPattern, int responseCode,
	...
	Latch    isStarted{2};
	ApnsMock apnsMock;
	auto     serverReturn = std::async(
        std::launch::async, [&apnsMock, responseCode, &responseBody, &reqBodyPattern, &isStarted, timeout]() {
            return apnsMock.exposeMock(responseCode, responseBody, reqBodyPattern, isStarted, timeout);
        });
