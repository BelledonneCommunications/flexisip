...
public:
	PresenceAuthListener(std::weak_ptr<belle_sip_main_loop_t> mainLoop, std::shared_ptr<PresentityPresenceInformation> info)
	: mMainLoop{std::move(mainLoop)}, mInfo{std::move(info)} {
		AuthDbBackend::get(); /*this will initialize the database backend, which is good to know that it works at startup*/
	}
	PresenceAuthListener(std::weak_ptr<belle_sip_main_loop_t> mainLoop, std::map<std::string,std::shared_ptr<PresentityPresenceInformation>> dInfo)
	: mMainLoop{std::move(mainLoop)}, mDInfo{std::move(dInfo)} {
		AuthDbBackend::get(); /*this will initialize the database backend, which is good to know that it works at startup*/
	}
	...
	void onResult(AuthDbResult result, const std::string &passwd) override {
		...
		auto mainloop = mMainLoop.lock();
		if (mainloop) {
			belle_sip_main_loop_cpp_do_later(mainloop.get(), func, "OnAuthListener to mainthread");
		}
	}

	void onResult(AuthDbResult result, const std::vector<passwd_algo_t> &passwd) override {
		onResult(result, passwd.front().pass);
	}
...
void PresenceAuthListener::processResponse(AuthDbResult result, const std::string &user) {
	...
	if (result == AuthDbResult::PASSWORD_FOUND) {
		...
		public:
			...
			void onRecordFound(const std::shared_ptr<Record> &record) {
				...
				for (auto extendedContact : record->getExtendedContacts()) {
