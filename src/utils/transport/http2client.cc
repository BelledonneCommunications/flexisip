...
void Http2Client::processGoAway() {
	...
	// move back all the non-treated requests into the pending queue
	...
	// disconnect and connect again if there still are requests to process
	disconnect();
...
void Http2Client::onStreamClosed(nghttp2_session& session, int32_t stream_id, uint32_t error_code) noexcept {
	...
	if (NGHTTP2_NO_ERROR == error_code) {
		...
		SLOGD << msg.str();
		if (context != nullptr) {
			context->getOnErrorCb()(context->getRequest(), error_code, msg.str());
			mActiveHttpContexts.erase(contextMapIterator);
		}
	}
}
