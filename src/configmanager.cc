...
ConfigManager::ConfigManager()
	...
	static ConfigItemDescriptor global_conf[] = {
	    ...
	    // Network settings.
	    {
	        ...
	        " - 'tls-client-connection': value in {'0', '1'}, the transport will only be used as a client "
	        "connection and will not listen to a port (defaults to '0'). It is the only mode where certificates are "
	        "not mandatory to create a transport and the global TLS parameters will not be used. It is still possible "
	        "to specify TLS parameters in the transport URI.\n"
	...
	static ConfigItemDescriptor cluster_conf[] = {
	    ...
	    },
	    {
	        ...
	        "SIP requests coming from these addresses won't be challenged by the authentication module and will not be "
	        "limited by the DosProtection module.",
	        "",
	    },
	    {
	        String,
