...
bool ApnsMock::exposeMock(int code, const string& body, const string& reqBodyPattern, bool timeout) {
	bool assert = false;
	try {
		boost::system::error_code ec;

		context tls(context::tls);
		tls.use_private_key_file("/home/anthony/source/flexisip/tester/cert/self_signed.pem", context::pem);
		tls.use_certificate_chain_file("/home/anthony/source/flexisip/tester/cert/self_signed_cert.pem");
...
std::function<void(const request &, const response &)> ApnsMock::handleRequest(int code, const string& body, const string& reqBodyPattern, bool& assert, bool timeout) {
