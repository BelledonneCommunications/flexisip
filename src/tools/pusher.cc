...
#define EQ1(i, name) (strcmp(name, argv[i]) == 0 && argc > i)
		for (int i = 1; i < argc; ++i) {
			if (EQ1(i, "--prefix")) {
				...
				foundLegfound_Legacy_Paramsacy_Params = true;
				...
				standardPushParams.pn-provider = argv[++i];
			} else if (EQ1(i, "--pn-prid")) {
				found_RFC_8599_Params = true;
				standardPushParams.pn-prid = argv[++i];
			} else if (EQ1(i, "--pn-params")) {
				found_RFC_8599_Params = true;
				standardPushParams.pn-params = argv[++i];
