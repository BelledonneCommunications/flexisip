...
struct Helper {
	...
	public:
		explicit MockedOutgoingTransaction(Agent* agent) : OutgoingTransaction(agent) {
		}
		...
		Agent* getAgent() noexcept override {
...
void forwardMidDialogRequestRouteIsNotUs() {
	...
	auto transaction = make_shared<Helper::MockedOutgoingTransaction>(proxy.getAgent().get());
