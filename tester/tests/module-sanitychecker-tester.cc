...
void stopWithError400AtRequestWithInvalidScheme() {
	...
	auto hooks = InjectedHooks{
	    ...
	    .onRequest =
	        [&]([[maybe_unused]] const auto& request) {
		        if (!requestIsValid) {
			        BC_FAIL("SanityChecker module must stop on invalid request");
		        }
		        return;
	        },
	};
...
void reply400ToInvalidRequest() {
	...
	// Valid URI in request
	{
		...
		request << "REGISTER sip:user@sip.example.org SIP/2.0\r\n"
		        ...
		        << "Content-Length: 0\r\n";
		auto event =
		    make_shared<RequestSipEvent>(proxyServer.getAgent(), make_shared<MsgSip>(0, request.str()), nullptr);
		sanityCheckerModule->processRequest(event);
		BC_ASSERT_FALSE(event->isTerminated());
	}
	// No via
	{
		stringstream request;
		...
		auto event =
		    make_shared<RequestSipEvent>(proxyServer.getAgent(), make_shared<MsgSip>(0, request.str()), nullptr);
		BC_ASSERT_THROWN(sanityCheckerModule->onRequest(event), InvalidRequestError);
	}
	// Invalid scheme in contact header
	{
		stringstream request;
